...
// Mirrors `ProgressStats` from `src/lib/progress-stats.ts` but
// with `createdAt` as ISO string. The Server Component converts
// Dates to strings before handing them to this client tree so we
// don't need to ship a Date polyfill or worry about RSC
// serialization edge cases.
export type ProgressViewStats = {
  summary: {
...
export function ProgressView({ stats }: { stats: ProgressViewStats }) {
  // Only piece of legitimately client state on this page: the
  // pagination cursor for "Recent attempts". Everything else is
  // derivable from props, so we keep them as plain consts (no
  // useState, no useEffect → no stale-state bugs, no extra renders).
  ...
  // Sort once per render of `attempts`. Cheap, but `useMemo`
  // makes intent explicit and protects against accidental
  // re-sorts in child renders.
    ...
    },
    {
      ...
      hint:
        sortedAttempts.length === 0
          ? "None yet"
          : `Last: ${new Date(sortedAttempts[0].createdAt).toLocaleDateString(
              "en-US",
            )}`,
    },
  ];
      ...
      <section className="relative overflow-hidden rounded-2xl border bg-gradient-to-br from-primary/15 via-background to-background p-6 sm:p-8">
        ...
        <div className="relative flex flex-col gap-2">
          ...
          <p className="max-w-2xl text-muted-foreground sm:text-lg">
            Track your accuracy and find your weakest vocab and kanji — all
            in one place.
          </p>
        </div>
      </section>
...
function RecentAttemptsCard({
  ...
  attempts: ProgressViewStats["attempts"];
  page: number;
  setPage: (n: number) => void;
}) {
  const totalPages = Math.max(1, Math.ceil(attempts.length / ATTEMPTS_PAGE_SIZE));
  const safePage = Math.min(Math.max(0, page), totalPages - 1);
  const start = safePage * ATTEMPTS_PAGE_SIZE;
  const slice = attempts.slice(start, start + ATTEMPTS_PAGE_SIZE);
            ...
            {totalPages > 1 && (
              <div className="mt-4 flex items-center justify-between gap-2 border-t pt-3 text-sm">
                <span className="text-muted-foreground tabular-nums">
                  Showing {start + 1}–
                  {Math.min(start + ATTEMPTS_PAGE_SIZE, attempts.length)} of{" "}
                  {attempts.length}
                </span>
